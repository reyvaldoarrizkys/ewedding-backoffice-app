...
<div class="max-w-2xl bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
    <div class="p-6 border-b border-slate-100 bg-slate-50/50">
        <h3 class="text-lg font-semibold text-slate-800 font-heading">Form Informasi</h3>
    </div>
    
    <form action="{{ route('information.store') }}" method="POST" class="p-6 space-y-5">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div>
                ...
                <select name="tipe_info" id="tipe_info" required
                    class="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-gold focus:border-gold outline-none transition-colors">
                    <option value="Pengumuman" {{ old('tipe_info') == 'Pengumuman' ? 'selected' : '' }}>Pengumuman Internal</option>
                    <option value="SOP" {{ old('tipe_info') == 'SOP' ? 'selected' : '' }}>Standar Operasional (SOP)</option>
                    <option value="Event" {{ old('tipe_info') == 'Event' ? 'selected' : '' }}>Event Tim</option>
                    <option value="Lainnya" {{ old('tipe_info') == 'Lainnya' ? 'selected' : '' }}>Lainnya</option>
                </select>
            ...
            <div>
                ...
                <input type="url" name="url_media" id="url_media" value="{{ old('url_media') }}"
                    class="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-gold focus:border-gold outline-none transition-colors"
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="konten_teks" id="konten_teks" rows="5"
                    class="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-gold focus:border-gold outline-none transition-colors">{{ old('konten_teks') }}</textarea>
