...
@section('header', 'Tambah Data Klien')

@section('content')
<div class="max-w-2xl bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
    <div class="p-6 border-b border-slate-100 bg-slate-50/50">
        <h3 class="text-lg font-semibold text-slate-800 font-heading">Form Klien Baru</h3>
    </div>
    ...
    <form action="{{ route('clients.store') }}" method="POST" class="p-6 space-y-5">
        ...
        <div>
            ...
            <input type="text" name="nama_klien" id="nama_klien" value="{{ old('nama_klien') }}" required
        ...
        <div>
            ...
            <textarea name="alamat" id="alamat" rows="3"
                class="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-gold focus:border-gold outline-none transition-colors">{{ old('alamat') }}</textarea>
